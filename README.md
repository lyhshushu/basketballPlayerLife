...
4. **逐场赛季**（新增）：每个赛季不再一步跳过——进入赛季页，每场比赛逐一模拟（生成比分 + 我的数据 + 双方全员技术统计），可按「模拟 1 场 / 5 场 / 快进整个赛季」控制节奏，实时查看进度、战绩、我的场均与最近比分
   - **队友数据**：赛季页可随时点开任意队友查看 13 项能力值与场均数据；点开任意一场比赛查看双方完整技术统计
5. **季后赛逐场推进**（新增）：常规赛战绩达标后进入季后赛，首轮/分区半决赛/总决赛三轮 BO7 逐场模拟，先赢 4 场晋级，一路打到夺冠
6. **赛季总结 + 属性升级**（新增）：整个赛季（含季后赛）结束后自动生成赛季总结卡（战绩、场均、完整奖项、成长点数与属性变化、年薪）；每赛季按表现获得成长点数，可在升级界面自由分配到三分/中投/终结/防守等 13 项属性
   - **完整奖项名单**：每个赛季结束自动生成全联盟奖项名单——MVP、得分王/篮板王/助攻王/抢断王/盖帽王、最佳新秀、最佳阵容一/二/三阵、最佳防守阵容、最佳防守球员等，获奖者带能力值与场均数据（你与联盟其他球员竞争）
   - **冲击 NBA**：不在 NBA 时，赛季总结提供「🚀 冲击 NBA」选项，按能力值判定成功率，成功即签约登陆 NBA
   - **属性年成长**：年轻时（≤24 岁爆发期）每年随机提升 2-4 项属性，35 岁后逐年随机减少且随年龄加重，直至退役（受潜力上限约束）
7. **合理薪资与合同**（新增）：现实 NBA 劳资风格——新秀合同按选秀顺位分档（状元/乐透/首轮/次轮），按能力签顶薪/准顶薪/大合同/中产/底薪；合同年限递增；**合同到期触发续约或自由市场**（可转会其他球队）
8. **交易系统**（新增）：
   - **赛季中随机交易**：模拟 NBA 交易逻辑，涉及能力>80 的球员变动时提示
   - **申请交易**：玩家可主动申请，按能力判定成功率
   - **休赛期交易 + 明星退役**：赛季结束后、进入下赛季前展示休赛期重磅交易与明星球员退役公告
   - **每年随机生成新秀**：休赛期生成一届选秀新秀（随机能力，偶有天才），下赛季注入各队阵容
9. **NBA 东西部 + 完整季后赛**（新增）：
   - 常规赛实时展示**东西部联盟排名**（胜负场、玩家高亮）
   - **附加赛**：常规赛胜率 45-55% 先打一场生死战，胜者进季后赛
   - 完整季后赛赛制：**分区首轮 → 分区次轮 → 分区决赛 → 总决赛**（BO7），带**对阵树图**
   - 非 NBA 联赛：半决赛 → 分区决赛 → 总决赛
10. **关键之战**：打进半决赛/总决赛、抢七、绝杀、关键罚球、国家队生死战时，可进入**逐回合单场模拟**，亲手把比赛打下来，结果直接影响生涯战绩
   - 事件卡出现「🎮 进入比赛模拟」，或直接选选项用原概率判定
   - 半决赛/总决赛赛季结束后，banner 上可「模拟关键战」或跳过
11. 生涯结束：自动生成可保存/分享的生涯战绩卡，写入历史档案与称号图鉴（战绩卡显示选秀顺位与 NCAA 季数）
